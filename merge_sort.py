...
def merge_sort_recursive(values):
  ...
  # We'll pass that list to a recursive call to the merge_sort_recursive function.
  left_values = merge_sort_recursive(values[:middle_index])
  # We'll also use slice syntax to get the right half of the list, and pass
  # that to merge_sort_recursive as well.
  right_values = merge_sort_recursive(values[middle_index:])
...
# this function is supposed to be computationally less expensive in terms of
# time complexity than the recursive implementation, because it avoids using 
# list slicing to create new sublists, which supposedly makes the recursive
# implementation have an extra factor of K time cost multiplying nlog(n),
# where K can be assumed to be no more than .25*n. 
def merge_sort_iterative(values):
    ...
    while width < n:
        for i in range(0, n, width):
            start_index = i
            midpoint = min(i + width, n)
            j = midpoint
            stop_index = min(i + width*2, n)
            temp = []
            while i < midpoint and j < stop_index:
                if arr[j] < arr[i]:
                    temp.append(arr[j])
                    j += 1
                else:
                    temp.append(arr[i])
                    i += 1
            temp.extend(arr[i:midpoint])
            temp.extend(arr[j:stop_index])
            arr[start_index:stop_index] = temp
        width *= 2
